...
describe('QueryGenerator', () => {
  describe('whereItemQuery', () => {
    ...
    it('should not parse any strings as aliases operators', function () {
      ...
      expect(() => QG.whereItemQuery('test', { $gt: 5 })).to.throw("Invalid value { '$gt': 5 }");

      expect(() => QG.whereItemQuery('test', { $between: [2, 5] })).to.throw("Invalid value { '$between': [ 2, 5 ] }");

      expect(() => QG.whereItemQuery('test', { $ne: 3 })).to.throw("Invalid value { '$ne': 3 }");

      expect(() => QG.whereItemQuery('test', { $not: 3 })).to.throw("Invalid value { '$not': 3 }");
    ...
    it('should parse set aliases strings as operators', function () {
      ...
      expect(() =>
        QG.whereItemQuery('OR', [{ test: { $gt: 5 } }, { test: { '!': 3 } }, { test: { [Op.in]: [4] } }])
    ...
    it('should correctly parse sequelize-dameng.where with .fn as logic', function () {
      ...
      QG.handleSequelizeMethod(this.sequelize.where(this.sequelize.col('foo'), Op.ne, null)).should.be.equal(
        'foo IS NOT NULL'
      );

      QG.handleSequelizeMethod(this.sequelize.where(this.sequelize.col('foo'), Op.not, null)).should.be.equal(
        'foo IS NOT NULL'
      );
    });
