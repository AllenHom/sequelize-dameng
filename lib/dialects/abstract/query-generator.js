...
class QueryGenerator {
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    for (const key in valueHash) {
      if (Object.prototype.hasOwnProperty.call(valueHash, key)) {
        ...
        if (modelAttributeMap && modelAttributeMap[key] && modelAttributeMap[key].autoIncrement === true && value == null) {
          if (!this._dialect.supports.autoIncrement.defaultValue || this.sequelize.options.dialect === 'dm') {
            fields.splice(-1, 1);
    ...
    if (this.sequelize.options.dialect === 'dm') {
  ...
  */
  ...
  quoteTable(param, alias) {
    ...
    // console.log(`~~~~~~~~~~quoteTable param ${param} alias ${alias}`);
    if (_.isObject(param)) {
      if (this._dialect.supports.schemas) {
        ...
        if (param.schema) {
          table += param.schema + (param.delimiter || '.'); Z;
        }
      ...
      table = this.getPrefix() + this.quoteIdentifier(param);
    }

    if (alias) {
      table += ` AS ${this.quoteIdentifier(alias)}`;
    }
    // console.log('~~~~~~~~~~quoteTable', table);
    return table;
  }
  ...
  */
  ...
  getPrefix() {
    let prefix = '';
    if (this.sequelize.options.dialect === 'dm') {
      prefix = this.sequelize.config.database + '.';
    }
    return prefix;
  }
