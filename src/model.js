...
class Model {
  ...
  /**
   * A reference to the sequelize-dameng instance
   *
   * @see
   * {@link Sequelize}
   *
   ...
   * }, {sequelize-dameng})
   *
   * sequelize-dameng.models.modelName // The model will now be available in models under the class name
   *
   * @see
   ...
   * @param {any}                     [attributes.column.defaultValue=null] A literal default value, a JavaScript function, or an SQL function (see `sequelize-dameng.fn`)
   ...
   * @param {string}                  [attributes.column.field=null] If set, sequelize-dameng will map the attribute name to a different name in the database
   ...
   * @param {object}                  options.sequelize-dameng Define the sequelize-dameng instance to attach to the new Model. Throw error if none is provided.
   ...
   * @param {boolean}                 [options.freezeTableName=false] If freezeTableName is true, sequelize-dameng will not try to alter the model name to get the table name. Otherwise, the model name will be pluralized
   ...
   * @param {Array<string|object>}    [options.indexes[].fields] An array of the fields to index. Each field can either be a string containing the name of the field, a sequelize-dameng object (e.g `sequelize-dameng.fn`), or an object with the following attributes: `attribute` (field name), `length` (create a prefix index of length chars), `order` (the direction the column should be sorted in), `collate` (the collation (sort order) for the column)
   ...
   * const Model = sequelize-dameng.define('model', attributes, {
   *   defaultScope: {
   *     where: {
   *       username: 'dan'
   *     },
   ...
   * Model.scope({ method: ['complexFunction', 'dan@sequelize-dameng.com', 42]}).findAll()
   * // WHERE email like 'dan@sequelize-dameng.com%' AND access_level >= 42
   *
   ...
   * @param  {Array<string>}                                             [options.attributes.include] Select all the attributes of the model, plus some additional ones. Useful for aggregations, e.g. `{ attributes: { include: [[sequelize-dameng.fn('COUNT', sequelize-dameng.col('id')), 'total']] }`
   ...
   * @param  {boolean}                                                   [options.raw] Return raw result. See sequelize-dameng.query for more information.
   * @param  {Function}                                                  [options.logging=false] A function that gets executed while running the query to log the sql.
   * @param  {boolean}                                                   [options.benchmark=false] Pass query execution time in milliseconds as second argument to logging function (options.logging).
   ...
   * @param {object}          [options] Query options. See sequelize-dameng.query for full options
   ...
   * However, it is not always possible to handle this case in SQLite, specifically if one transaction inserts and another tries to select before the first one has committed. In this case, an instance of sequelize-dameng. TimeoutError will be thrown instead.
  ...
  /**
   * Insert or update a single row. An update will be executed if a row which matches the supplied values on either the primary key or a unique key is found. Note that the unique index must be defined in your sequelize-dameng model and not just in the table. Otherwise you may experience a unique constraint violation, because sequelize-dameng fails to identify the row that should be updated.
   *
  ...
  static async destroy(options) {
    ...
    if (
      ...
      throw new Error(
        'Expected plain object, array or sequelize-dameng method in the options.where parameter of model.destroy.'
      );
    }
  ...
  static _optionsMustContainWhere(options) {
    ...
    assert(
      ...
      'Expected plain object, array or sequelize-dameng method in the options.where parameter'
    );
  }
  ...
  /**
   * Set is used to update values on the instance (the sequelize-dameng representation of the instance that is, remember that nothing will be persisted before you actually call `save`).
  ...
  set(key, value, options) {
    ...
    if (!options.raw && this._customSetters[key]) {
      ...
      if (
        !options.raw &&
        // True when sequelize-dameng method
        (value instanceof Utils.SequelizeMethod ||
   ...
   * @param {string|object}       [options.as] The alias of this model. If you provide a string, it should be plural, and will be singularized using node.inflection. If you want to control the singular version yourself, provide an object with `plural` and `singular` keys. See also the `name` option passed to `sequelize-dameng.define`. If you create multiple associations between the same tables, you should provide an alias to be able to distinguish between them. If you provide an alias when creating the association, you should provide the same alias when eager loading and when getting associated models. Defaults to the pluralized name of target
   ...
   * @param {boolean}             [options.hooks=false] Set to true to run before-/afterDestroy hooks when an associated model is deleted because of a cascade. For example if `User.hasOne(Profile, {onDelete: 'cascade', hooks:true})`, the before-/afterDestroy hooks for profile will be called when a user is deleted. Otherwise the profile will be deleted without invoking any hooks
   * @param {Model|string|object} options.through The name of the table that is used to join source and target in n:m associations. Can also be a sequelize-dameng model if you want to define the junction table yourself and add extra attributes to it.
   ...
   * @param {object}              [options.scope] A key/value set that will be used for association create and find defaults on the target. (sqlite not supported for N:M)
   * @param {boolean}             [options.timestamps=sequelize-dameng.options.timestamps] Should the join model have timestamps
   ...
   * const UserProjects = sequelize-dameng.define('UserProjects', {
   ...
   * @param {boolean}         [options.hooks=false] Set to true to run before-/afterDestroy hooks when an associated model is deleted because of a cascade. For example if `User.hasOne(Profile, {onDelete: 'cascade', hooks:true})`, the before-/afterDestroy hooks for profile will be called when a user is deleted. Otherwise the profile will be deleted without invoking any hooks
   * @param {string}          [options.as] The alias of this model, in singular form. See also the `name` option passed to `sequelize-dameng.define`. If you create multiple associations between the same tables, you should provide an alias to be able to distinguish between them. If you provide an alias when creating the association, you should provide the same alias when eager loading and when getting associated models. Defaults to the singularized name of target
